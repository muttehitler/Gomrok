...
export default function Order() {
    ...
    useEffect(() => {
        (async () => {
            ...
            const result = JSON.parse(await getOrderList({ csrf: generateCsrfToken(await getCookieCSRF() ?? ''), startIndex: (currentPage - 1) * pageSize, limit: pageSize, order: -1 }))
        ...
        <Page back={true}>
            ...
            <div className='container'>
                ...
                {orders.length > 0 ? orders.map(x =>
                    (<OrderItem id={x.id} name={x.name} product={x.product} payed={x.payed} price={x.price} finalPrice={x.finalPrice} key={x.name} orderDetailUrl={'/admin/order/' + x.id} />)
                ) :
                    (<div role="status">
