...
export async function createInvoice(data: any) {
    ...
    const response = await axios.post(process.env.API_ADDRESS + PAYMENT_PATTERNS.CREATE_INVOICE, data, {
        headers: {
...
export async function getInvoice(data: any) {
    validateCsrfTokenWithEx(data.csrf, (await cookies()).get('csrf')?.value ?? '')
    const response = await axios.get(process.env.API_ADDRESS + PAYMENT_PATTERNS.GET + data.id, {
        headers: {
            'accept': 'application/json',
            'Content-Type': 'application/json',
            'authorization': 'Bearer ' + (await cookies()).get('token')?.value
