...
import { MARZBAN_PANEL_PATTERNS } from "@app/contracts/patterns/marzbanPanelPattern";

@Injectable()
export default class MarzbanPanel extends PanelBase {
    ...
    async resetUsage(user: string, panelId: string) {
        ...
        const response = await firstValueFrom(this.httpService.post(panel.url + MARZBAN_PANEL_PATTERNS.USER.RESET.replace('{username}', user), {}, {
            headers: {
    ...
    async modifyUser(user: PanelModifyUserDto, panelId: string) {
        ...
        const response = await firstValueFrom(this.httpService.put(panel.url + MARZBAN_PANEL_PATTERNS.USER.MODIFY + user.username, {
            on_hold_timeout: user.activationDeadline,
            on_hold_expire_duration: user.usageDuration,
            data_limit: user.dataLimit,
            data_limit_reset_strategy: user.dataLimitResetStrategy,
            note: user.note,
            expire: user.expireDate ? Math.floor(new Date(user.expireDate).getTime() / 1000) : 0,
    ...
    async testConnection(panelDto: AddPanelDto) {
        ...
        const response = this.httpService.post(panelDto.url + MARZBAN_PANEL_PATTERNS.ADMIN.TOKEN, data, { headers, validateStatus: () => true })
    ...
    async addUser(user: PanelAddUserDto, panelId: string): Promise<ResultDto> {
        ...
        let inbounds = await firstValueFrom(this.httpService.get(panel.url + MARZBAN_PANEL_PATTERNS.SYSTEM.INBOUNDS, {
            headers: {
                'Authorization': 'Bearer ' + auth,
                'Content-Type': 'application/json',
                'accept': 'application/json'
        ...
        let proxies = Object.fromEntries(Object.entries(inbounds.data).map(([key, value]: [string, any[]]) => [key, {}]))
        let uinbounds = Object.fromEntries(Object.entries(inbounds.data).map(([key, value]: [string, any[]]) => [key, value.map(x => x.tag)]))
        ...
        const response = await firstValueFrom(this.httpService.post(panel.url + MARZBAN_PANEL_PATTERNS.USER.ADD, userData,
            {
                headers: {
                    'Authorization': 'Bearer ' + auth,
                    'Content-Type': 'application/json',
    ...
    async getUser(user: string, panelId: string): Promise<DataResultDto<PanelUserDto>> {
        ...
        const response = await firstValueFrom(this.httpService.get(panel.url + MARZBAN_PANEL_PATTERNS.USER.GET + user, {
            headers: {
                'Authorization': 'Bearer ' + auth,
                'Content-Type': 'application/json',
                'accept': 'application/json'
        ...
        return {
            ...
            data: {
                ...
                activated: response.data.status == 'active',
    ...
    async revokeSubscription(user: string, panelId: string): Promise<DataResultDto<PanelUserDto>> {
        ...
        const response = await firstValueFrom(this.httpService.post(panel.url + MARZBAN_PANEL_PATTERNS.USER.REVOKE_SUB.replace('{username}', user), {
        }, {
            headers: {
                'Authorization': 'Bearer ' + auth,
                'Content-Type': 'application/json',
    ...
    async enableUser(user: string, panelId: string): Promise<ResultDto> {
        ...
        const response = await firstValueFrom(this.httpService.post(panel.url + MARZBAN_PANEL_PATTERNS.USER.MODIFY.replace('{username}', user),
            {
                status: 'active'
            }, {
            headers: {
    ...
    async getToken(httpService: HttpService, panel: Panel): Promise<PanelTokenFuncDto> {
        ...
        const response = (await firstValueFrom(httpService.post(panel.url + MARZBAN_PANEL_PATTERNS.ADMIN.TOKEN, data, { headers, validateStatus: () => true })))
